...
xd = ($xrange/4).to_i
yd = ($yrange/4).to_i
rocktest = Fighter.new("Scissors", (2*xd),(1*yd),"Alive")
rocktest2 = Fighter.new("Rock", (3*xd),(1*yd),"Alive")
papertest = Fighter.new("Paper", (1*xd), (1*yd),"Alive")
...
while count < ($xrange*$yrange)
   #  puts  hashgrid["#{countx},#{county}"][2]
    ...
    if pngarray[count] == "g"
        ...
        pngx[countx , county] = ochrey
        #Fighter.new("Yor", 0, 0 , 0, [fuzzer(countx, $xrange), $xrange].min,[fuzzer(county, $yrange), $yrange].min , "Alive", 0)
        ...
        pngx[countx , county] = black
       # Fighter.new("Bor", 0, 0 , 0, [fuzzer(countx, $xrange), $xrange].min,[fuzzer(county, $yrange), $yrange].min , "Alive", 0)
       else
            pngx[countx , county] = black
        end
        countx+=1
